...
# ============================================================
# Biopython is NOT pre-installed on Kaggle GPU images (Python 3.12).
# We install from .whl files uploaded as a Kaggle dataset.
# This works with Internet OFF (no PyPI access needed).
# The code auto-detects the Python version and picks the right wheel.
# Kaggle sometimes appends ' (1)' to filenames — we clean that.
# ============================================================
...
try:
    ...
    # Search all attached datasets for .whl files
    wheels = sorted(glob.glob('/kaggle/input/**/biopython*.whl', recursive=True))
    print(f"Found wheels: {wheels}")

    # Copy to /kaggle/working/ with clean names (strip ' (1)' suffixes)
    clean_wheels = []
    for whl in wheels:
    ...
    if clean_wheels:
        # Pick wheel matching our Python version (e.g., cp312 for Python 3.12)
        ...
        # Fallback: try pip install (requires Internet ON)
